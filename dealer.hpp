...
namespace {
    ...
    auto create_half_deck( T && suits) {
        ...
        for(const auto & aSuit : suits) {
            std::transform(std::begin(rank), std::end(rank), std::back_inserter(deck), [suit = aSuit](std::string aRank){return suit + aRank;});
        }
        return deck;
    }
    ...
    auto create_akakuro_decks() {   // FIXME: need refactoring
        ...
        const auto suits_black = strVec{ "S"s, "C"s };
